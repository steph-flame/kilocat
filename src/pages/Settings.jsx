      ...
      <div className="max-w-xl mx-auto px-4 py-6 sm:py-8">
        ...
        <section style={{ background: C.card, borderColor: C.line }} className="border rounded-2xl p-4 sm:p-5 mb-4">
          <div className="flex items-center justify-between mb-1">
            ...
            <div className="flex rounded-full overflow-hidden border" style={{ borderColor: C.line }}>
              {["v3", "v4", "v5", "v2", "v1"].map((a) => (
                <button key={a} onClick={() => setEstimator(a)} aria-pressed={estimator === a} style={{ background: estimator === a ? C.spruce : "transparent", color: estimator === a ? "#fff" : C.sub }} className="text-xs px-2.5 py-1.5 font-mono">{a}{a === "v3" ? " · recommended" : (a === "v4" || a === "v5") ? " · beta" : ""}</button>
              ))}
            </div>
          </div>
